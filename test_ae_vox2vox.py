...
from ae_vox2vox_test import AE_vox2vox
import psutil
import gc
import resource
sys.path.append(os.path.join(BASE_DIR, 'data'))
from write_tfrecords_vox import get_models, read_bv
...
def test(ae):
    ...
    model_ids = []
    listFile = os.path.join("./data/render_scripts/lists/{}_lists/{}_idx.txt".format(category_name, split))
    ...
    for model_id in model_ids:
        ...
        recon_losses.append(recon_loss)
        IoUs.append(iou)
        print('model_id: {}, loss: {}, IoU: {}'.format(model_id, recon_loss, iou))
