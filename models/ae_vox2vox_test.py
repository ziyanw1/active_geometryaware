...
class AE_vox2vox(object):
    def __init__(self, FLAGS):
        self.FLAGS = FLAGS

        self.is_training = tf.placeholder(tf.bool, shape=(), name='is_training')
        self.voxel_batch = tf.placeholder(dtype=tf.float32, 
            shape=(1, FLAGS.voxel_resolution, FLAGS.voxel_resolution, FLAGS.voxel_resolution, 1))
    ...
    def _create_network(self):
        with tf.device('/gpu:0'):
            ...
            if self.FLAGS.network_name == 'ae':
                self.z_vox = self._create_encoder(self.voxel_batch, trainable=True, if_bn=True, scope_name='vox_ae_encoder') 
            ...
            if self.FLAGS.use_gan:
                self.fake_pred, _ = self._create_discriminator(self.preds, scope_name='vox_discriminator')
                self.real_pred, _ = self._create_discriminator(self.voxel_batch, reuse=tf.AUTO_REUSE, scope_name='vox_discriminator')
