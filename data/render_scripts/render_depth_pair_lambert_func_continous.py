...
# scp -r jerrypiglet@128.237.184.26:Bitsync/3dv2017_PBA/data/render_scripts/render_depth_pair_lambert* . && nice -n 10 blender blank.blend -b -P render_depth_pair_lambert_main.py -- 02958343 ../blender_renderings/02958343_select.list 512 10
...
def cameraExtrinsicMatrix(q,camPos):
	...
	return Rt, R_world2bcam ,q_world2bcam,t_world2bcam
...
azim0 = np.float(np.random.randint(low=30, high=330))
elev0 = np.float(np.random.randint(low=10, high=45))
theta = 0.
# azim_all = azim0 + np.random.normal(loc=0., scale=10., size=(VIEWS,))
azim_all = azim0 + np.linspace(0, 90, VIEWS)
azim_all[0] = azim0
...
for bkg in range(2):
	for res in res_list:
		...
		for i in range(VIEWS):
			...
			# q = np.nan_to_num(q)
			Rt,R_extr, q_extr,t_extr = cameraExtrinsicMatrix(q,camPos)
