...
# scp -r jerrypiglet@128.237.184.26:Bitsync/3dv2017_PBA/data/render_scripts/render_depth_pair_lambert* . && nice -n 10 blender blank.blend -b -P render_depth_pair_lambert_main.py -- 02958343 ./lists/02958343_select.list 128 200
...
def objectCenteredCamPos(rho,azim,elev, t1, t2, t3):
    phi = np.deg2rad(elev)
...
def cameraExtrinsicMatrix(q,camPos, t1, t2, t3):
...
#       - right-handed: positive z look-at direction
    ...
    t_bcam2world = -1*R_bcam2world.dot(np.array([[-t1], [-t2], [-t3]])) # [[0], [0], [-1]]
...
azim_all = azim0 + np.linspace(0, 360, 10)
azim_all[0] = azim0
t1_0 = random.uniform(-0.1, 0.1)
t2_0 = random.uniform(-0.1, 0.1)
t3_0 = random.uniform(0.7, 1.1)
...
for bkg in range(1):
    for res in res_list:
        ...
        for i in range(VIEWS):
            azim = azim_all[i] + random.uniform(-1, 1)
            elev = elev0 + random.uniform(-1, 1)
            t1 = t1_0 + random.uniform(-0.03, 0.03)
            t2 = t2_0 + random.uniform(-0.03, 0.03)
            t3 = t3_0 + random.uniform(-0.01, 0.01)
            theta = theta0 + random.uniform(-1, 1)
            camPos = objectCenteredCamPos(rho,azim,elev, t1, t2, t3)
            q1 = camPosToQuaternion(camPos)
            q2 = camRotQuaternion(camPos,theta)
            q = quaternionProduct(q2,q1)
            Rt,R_extr, q_extr,t_extr, t_bcam2world = cameraExtrinsicMatrix(q,camPos, t1, t2, t3)
            ...
            shutil.copyfile(scene.render.filepath,"{0}/{1}_{2}.png".format(save_path,i,bkg))
            ...
            t_list.append(np.array([t1, t2, t3]))
            # print([np.array(q_extr),t_extr.squeeze()])

        trans_path = "{0}/trans_per.mat".format(save_path)
        scipy.io.savemat(trans_path,{'angles_list':np.stack(angles_list), 'R_list':np.stack(R_list), \
            'q_extr_list':np.stack(q_extr_list), 't_extr_list':np.array(t_extr_list), 't_list':np.array(t_list)})
