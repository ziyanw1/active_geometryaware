...
def reproject_all(views, depths, masks):
    with tf.name_scope('reproject_lift'):
        ...
        views = map(f, tf.unstack(views, axis=3))
        masks = map(f, tf.unstack(masks, axis=3))
        ...
        exclude_pts = map(lambda x: tf.concat(x, axis=1), exclude_pts)
    ...
    with tf.name_scope('reproject_sink'):
        ...
        reprojs, depths, alphas, viss = zip(*outputs)
